#!/usr/bin/env bash
# Idempotent copy installer for the waybar-todo unit.
# Usage: ./install.sh [--remove]
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
UNIT_NAME="waybar-todo"

# shellcheck source=env.conf
source "$SCRIPT_DIR/env.conf"
INSTALL_ROOT="${INSTALL_ROOT:-$HOME/.config/acw/waybar-todo}"
...
install_files() {
    ...
    echo "Done. Add the custom/todo module fragment to your waybar config (see README)."
}
