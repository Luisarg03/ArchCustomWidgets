#!/usr/bin/env bash
# Idempotent copy installer for the waybar-countdown unit.
# Usage: ./install.sh [--remove]
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
UNIT_NAME="waybar-countdown"

# shellcheck source=env.conf
source "$SCRIPT_DIR/env.conf"
INSTALL_ROOT="${INSTALL_ROOT:-$HOME/.config/acw/waybar-countdown}"
...
install_files() {
    ...
    echo "Done. Add the custom/countdown module fragment to your waybar config (see README)."
}
