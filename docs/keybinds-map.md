...
Authoritative map of LIVE keybinds, verified 2026-08-14 (re-checked 2026-08-29 and 2026-09-15) via `hyprctl binds` (111 distinct live binds) + config inventory. **Check this file before assigning any new key combo.** Re-run `hyprctl binds -j | jq` after any Hyprland config change and update the date above.

## How to read this

- **Entrypoint is Lua**: the compositor reports `configProvider: lua` (Hyprland 0.56.1) and **every** live bind has `dispatcher: __lua` in `hyprctl binds -j`. `~/.config/hypr/hyprland.lua` loads the layers with `require()`:
  - `require("keybindings")` → `~/.config/hypr/keybindings.lua` (user layer)
  - `require("custom.keybinds")` → `~/.config/hypr/custom/keybinds.lua` (custom layer, loaded last — wins on conflicts)
- **Edit the `.lua` files.** `~/.config/hypr/hyprland.conf` still contains its `source =` lines, but that file is no longer the entrypoint, so any bind added to `keybindings.conf` / `custom/keybinds.conf` is **dead**. The `.conf` twins are legacy mirrors kept in sync by hand (they were generated by hyprconf2lua v1.4.0); the `Source` column below points at those mirrors, which is why its line numbers are approximate for the live `.lua` files.
- **DEAD** binds: `~/.config/hypr/hyprland/keybinds.conf` (Caelestia's file) is loaded by no layer. Do not use combos only claimed there as "free" — they are reserved intent, and would conflict if that file is ever re-enabled.
...
| Combo | Unit | Action |
|---|---|---|
| Ctrl+Super+G | task-notes | capture note (qs -p capture.qml) — added 2026-08-14; was Super+G until conflict found (Super+G = togglegroup). Live: `custom/keybinds.lua:61` |
| Super+H | recruiter-drafts | job-posting popup (qs -p capture.qml) — added 2026-09-15; free in every layer before that. Live: `custom/keybinds.lua:65` |

## Verification command

```bash
hyprctl systeminfo | grep configProvider   # expect: configProvider: lua
hyprctl binds -j | jq -r '.[] | "\(.modmask)+\(.key) -> \(.dispatcher) \(.arg)"'
```

Run this before assigning any new combo. `.dispatcher` is `__lua` for every live bind (the `.handler` field of older Hyprland releases no longer exists).
